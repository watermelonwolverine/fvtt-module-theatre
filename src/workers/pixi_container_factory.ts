...
export default class _TheatrePixiContainerFactory {
    ...
    createPortraitPIXIContainer(
        ...
        this.theatre._addSpritesToPixi(imgSrcs, (loader, resources) => {
            ...
            let portWidth = (ename && params.emotes[ename] && insertResource) ?
              <number>resources[insertResource]?.texture?.width : <number>resources[imgPath]?.texture.width;
            ...
            if (!ename) {
                ...
                if (params.emotes[ename] && insertResource) {
                    dockContainer.x = isLeft ?
                        (-1 * portWidth) : (Number(this.theatre.stage.theatreDock?.offsetWidth) + portWidth);
                    this.portrait_container_setup.setupPortraitContainer(imgId, optAlign, insertResource, resources, true);
                } else {
                    dockContainer.x = initX;
                    this.portrait_container_setup.setupPortraitContainer(imgId, optAlign, imgPath, resources, true);
                }
